...
**Описание:**
Игрок отправляет обновленное состояние после хода, и сервер отвечает возможными ходами для каждой фигуры.

**Тело запроса:** 
- `gameId`(String): Идентификатор игры.
- `board`(Array): Массив с текущим состоянием всех фигур на доске.
- `pieces` (Array): Список фигур с их статусами и позициями.
- `turn` (String): Информация о том, чей ход следующий.
- 
**Пример тела запроса:**
```json
{
...
**Ожидаемый ответ:**
- 200 OK: Ход успешно обработан, возвращены потенциальные ходы.
- 400 Bad Request: Некорректный запрос.
- 401 Unauthorized: Пользователь не авторизирован.
- 404 Not Found: Игровая сессия или игра не найдена.
...
### 7. API уведомлений и/или событий:
...
**Параметры**:  
- `gameId`: Идентификатор игровой сессии, которую необходимо завершить.
